...
const SliderImages = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        let res = await axios.get("/settings/sports-sliders");
        if (res?.data?.success) {
  ...
  const formSubmitHandler = async (event) => {
    ...
    try {
      ...
      setIsLoading(true);
      const res = await axios.post(`/settings/add/sports-slider`, obj);
      if (res?.data?.success) {
        setUrl("");
        setFileList([]);
  ...
  return (
    <div className="m-sm-30">
      <div className="mb-sm-30">
        <Breadcrumb routeSegments={[{ name: "Sports Slider Image" }]} />
      </div>

      <Grid container>
        <Grid item md={6} xs={12} className="mb-4">
          <Card elevation={3}>
            <CardHeader title="Add New Sports Slider Image" />
        ...
        <Grid item md={10} xs={12}>
          <Card elevation={3}>
            <CardHeader title="All Sports Slider Images" />
            {!isPageLoading ? (
